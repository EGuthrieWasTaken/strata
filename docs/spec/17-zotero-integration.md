...
users, and is genuinely good at three jobs `strata` has deliberately declined to
do:

1. **Holding the PDFs.** Storage, sync, full-text indexing, and a built-in reader
   with highlights and notes. `strata` refuses to commit full texts
...
`strata` never stores a PDF; Zotero never stores a screening decision. The link
...
|---|---|---|
| Zotero → `strata` | Import a collection or saved search as records | M6 |
| Zotero → `strata` | Resolve and open the full text during screening and extraction | M6 |
| `strata` → Zotero | Push included studies to a collection for citation | M6 |
...
None of this is required for a complete review. `strata` MUST remain fully
...
which fits `strata`'s offline-by-default posture ([13 §4](13-nonfunctional.md)).
...
`keyring`, never in `strata.toml` and never in the repository. If no keychain is
available, `strata` MUST read it from an environment variable and MUST say that it
...
Matching a Zotero item to a `strata` record uses the same canonical-key ladder as
...
**Zotero is mutable; `strata` is append-only.** The two cannot be kept in
...
- Conflicts are resolved in favour of **Zotero for bibliographic metadata** and
  **`strata` for anything decision-shaped**. Neither system is ever authoritative
  over the other's domain.

## 5. Commands

```
strata zotero link                       # authenticate / detect, choose a library
strata zotero pull --collection "Spacing review" --search S-05-zotero
strata zotero push --set included --collection "Spacing review / Included"
strata zotero status                     # mapped, unmapped, stale links
strata zotero open <record-id>           # open the full text in Zotero's reader
```

`strata zotero pull` is an import source like any other
...
delayed test" in the PDF and have `strata` offer it as the source locator for the
